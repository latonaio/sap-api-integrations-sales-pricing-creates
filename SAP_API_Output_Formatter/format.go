...
func ConvertToSalesPricingConditionValidity(raw []byte, l *logger.Logger) (*SalesPricingConditionValidity, error) {
	...
	if err != nil {
		return nil, xerrors.Errorf("cannot convert to PricingConditionValidity. raw data is:\n%v\nunmarshal error: %w", string(raw), err)
	}
	data := pm.D
...
func ConvertToSalesPricingConditionRecord(raw []byte, l *logger.Logger) (*SalesPricingConditionRecord, error) {
	pm := &responses.SalesPricingConditionRecord{}
	err := json.Unmarshal(raw, pm)
	if err != nil {
		return nil, xerrors.Errorf("cannot convert to ToConditionRecord. raw data is:\n%v\nunmarshal error: %w", string(raw), err)
	}
	data := pm.D
	conditionRecord := &SalesPricingConditionRecord{
		ConditionRecord:              data.ConditionRecord,
